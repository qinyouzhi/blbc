function drawCircle(_options) {
	...
	options.lineWidth = options.lineWidth || 3; //定义默认圆描边的宽度;
	...
	ctx.font = 'normal normal bold  Arial'; //定义字体加粗大小字体样式;
	ctx.fillStyle = options.color == 'grd' ? grd : '#333'; //判断文字填充样式为颜色，还是渐变色;
	ctx.fillText((options.angle * 100) + '%', sCenter, sCenter + 5); //设置填充文字;
